...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
            InlineKeyboardButton('🔎 Search', url=f'http://t.me/{temp.U_NAME}?startgroup=true')
        ],[
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        for msg in msgs:
            ...
            try:
                ...
                filesarr.append(msg)
                k = await client.send_message(chat_id = message.from_user.id, text=f"𝗧𝗛𝗜𝗦 𝗙𝗜𝗟𝗘 𝗪𝗜𝗟𝗟 𝗕𝗘 𝗗𝗘𝗟𝗘𝗧𝗘𝗗 𝗜𝗡\n𝟭𝟬 𝗠𝗜𝗡𝗦 (Due to copyright issues)   ⌛\n\n𝗙𝗼𝗿𝘄𝗮𝗿𝗱 𝘁𝗼 𝘀𝗮𝘃𝗲𝗱 𝗺𝗲𝘀𝘀𝗮𝗴𝗲𝘀 𝗼𝗿 𝘀𝗮𝘃𝗲 & 𝘀𝘁𝗮𝗿𝘁 𝗱𝗼𝘄𝗻𝗹𝗼𝗮𝗱")
    ...
    if data.startswith("sendfiles"):
        ...
        k = await client.send_message(chat_id=message.from_user.id,text=f"<b>Get All Files in a Single Click!!!\n\n📂 ʟɪɴᴋ ➠ : {g}\n\n<i>Note: This message is deleted in 5 mins to avoid copyrights. Save the link to Somewhere else</i></b>", reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton('Download File ⬇️', url=g)
                    ], [
        ...
        k = await client.send_message(chat_id=user,text=f"<b>Name 📛: <code>{files.file_name}</code> \n\nSize 📦 ➠ : {get_size(files.file_size)}\n\nDownload link ⬇️ : {g}\n\n<i>Note ⚠️</i>: This link will be deleted in 20 mins to avoid copyrights</b>", reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton('Download File ⬇️', url=g)
                    ], [
        ...
        for file in files:
            ...
            if STREAM_MODE == True:
                button = [[
                    ...
                    InlineKeyboardButton('🔎 Search Again', url="https://telegram.me/yedekho")
                ],[
        ...
        if settings['is_shortlink'] and not await db.has_premium_access(user):
            ...
            files = files_[0]
            g = await get_shortlink(chat_id, f"https://telegram.me/{temp.U_NAME}?start=file_{file_id}")
            k = await client.send_message(chat_id=message.from_user.id,text=f"<b>Name 🔖 : <code>{files.file_name}</code> \n\nDownload Link 👇🏻🗳️ : {g}\n\nSize 📦 : {get_size(files.file_size)}\n\n<i>Note ⚠️</i>: This link will be deleted in 20 minutes to avoid copyrights</b>", reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton('Download file ⬇️', url=g)
                        ], [
    ...
    if not files_:
        ...
        try:
            ...
            k = await msg.reply("𝗧𝗛𝗜𝗦 𝗙𝗜𝗟𝗘 𝗪𝗜𝗟𝗟 𝗕𝗘 𝗗𝗘𝗟𝗘𝗧𝗘𝗗 𝗜𝗡\n𝟭𝟬 𝗠𝗜𝗡𝗦 (Due to copyright issues)   ⌛\n\n𝗙𝗼𝗿𝘄𝗮𝗿𝗱 𝘁𝗼 𝘀𝗮𝘃𝗲𝗱 𝗺𝗲𝘀𝘀𝗮𝗴𝗲𝘀 𝗼𝗿 𝘀𝗮𝘃𝗲 & 𝘀𝘁𝗮𝗿𝘁 𝗱𝗼𝘄𝗻𝗹𝗼𝗮𝗱", quote=True)
            await asyncio.sleep(600)
            await msg.delete()
